...
begin
	...
	result := result + '"users":' + json_encode( _getUserNames() );
	result := result + '}';
end;

function TPam2Group._getUserNames(): TStrArray;
var i: Integer;
    len: Integer;
    users: TPam2UGBinding_List;
begin
	users := db.getUGBindings(self);
	len := length(users);
	setLength( result, Len );

	for i := 0 to len - 1 do
		result[ i ] := users[i].user.loginName;
